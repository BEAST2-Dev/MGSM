...
public class MGSMBeagleTreeLikelihood extends TreeLikelihood {
    ...
    private double [][] currentCategoryRatesPerNode;
...
//                preferenceFlags |= BeagleFlag.SCALING_MANUAL.getMask();
        ...
        if (hasNodeIndependentCategories(m_siteModel)) {
        	...
        	currentCategoryRatesPerNode = new double[m_nNodeCount][categoryCount];
        	//storedCurrentCategoryRates = new double[categoryRates.length * m_nNodeCount];
        }
        ...
        for (int i = 0; i < eigenCount; i++) {
            if (branchUpdateCount[i] > 0) {
            	if (hasNodeIndependentCategories(m_siteModel)) {
            		...
            		int matrixSize = m_nStateCount * m_nStateCount;
            		double [] transitionMatrix = new double[matrixSize * categoryCount];
            		double [] matrix = new double[matrixSize];
            		Node [] nodes = treeInput.get().getNodesAsArray();
                    int offsetIndex = eigenBufferHelper.getOffsetIndex(0);
            		for (int j = 0; j < branchUpdateCount[i]; j++) {
            			Node node = nodes[matrixUpdateIndices[i][j] % m_nNodeCount];
	                    beagle.setCategoryRates(currentCategoryRatesPerNode[node.getNr()]);
		                beagle.updateTransitionMatrices(
		                        offsetIndex,
		                        new int[]{matrixUpdateIndices[i][j]},
		                        null,
		                        null,
		                        new double[]{branchLengths[i][j]},
		                        1);
	                    
//	                    double [] rates = m_siteModel.getCategoryRates(node);
//            			double rate = branchRateModel.getRateForBranch(node);
//            			for (int k = 0; k < categoryCount; k++) {
//            				substitutionModel.getTransitionProbabilities(node, node.getParent().getHeight(), node.getHeight(), rate * rates[k], matrix);
//            				System.arraycopy(matrix, 0, transitionMatrix, k * matrixSize, matrix.length);
//            			}
//            			beagle.setTransitionMatrix(matrixUpdateIndices[i][j], transitionMatrix, 1.0);
            		}
            	}
            }
        }
